...
HDFS_BASE = "hdfs://dtm-0.svc-dt.dt.svc.superman.k8s:8020"
DATASET_PATH = f"{HDFS_BASE}/dataset/parquet_2"
...
def get_route_sequence(origin, destination):
    ...
    if orig_seq_id is None or dest_seq_id is None:
        print("no network structure been found")
        sys.exit(1)
...
def main():
    ...
    # Time Spark connection setup
    spark_conn_start = time.time()
    spark = SparkSession.builder \
        .appName("TDCS_TravelTime_Calculator") \
        .getOrCreate()
    # [改善點 7] 開啟 Arrow-based 傳輸，加速後續 toPandas() 的序列化效率
    spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true")
    spark_conn_time = round(time.time() - spark_conn_start, 2)
    print(f"[*] Spark connection time: {spark_conn_time} seconds")

    try:
        # Phase 1: Route Sequence (Triggers HDFS CSV read & collect)
        hdfs_load_start1 = time.time()
        route_gantries = get_route_sequence(args.origin, args.destination)
        hdfs_load_time1 = time.time() - hdfs_load_start1
        print(f"[*] Metadata load time: {round(hdfs_load_time1, 2)} seconds")
        
        # Phase 2: Date Filtering
        target_weekday, weekday_dates = parse_weekday_input(args.weekday)
        if target_weekday is not None:
            query_mode = 'weekday'
            matching_dates = weekday_dates
            print(f"[*] 判定為「星期幾」模式：weekday = {target_weekday}，"
                f"資料範圍內共 {len(matching_dates)} 天符合。")
        else:
            query_mode = 'dates'
            matching_dates = [d.strip() for d in args.weekday.split(",")]
            print(f"[*] 判定為「明確日期清單」模式，共 {len(matching_dates)} 天。")

        # [改善點 6] 事先解析並對齊出發時間，供 Phase 3 的 12 小時時間窗口過濾使用
        depart_hour, depart_minute, _ = parse_and_align_depart_time(args.time)

        # 提早算出路段名稱清單，讓 route / vehicle_type 這兩個選擇性最高的過濾條件
        # 能在 Phase 3 讀取 Parquet 時就先套用，而不是等到 Phase 4 才做
        segment_names = build_segment_names(route_gantries)

        # Phase 3: Parquet Loading & Caching (Triggers HDFS Parquet read & count)
        hdfs_load_start2 = time.time()
        lookup_df = load_parquet_lookup_df(
            spark, query_mode, target_weekday, matching_dates,
            depart_hour, depart_minute, segment_names,
            vehicle_type=31, window_hours=12
        )
        hdfs_load_time2 = time.time() - hdfs_load_start2
        print(f"[*] HDFS dataset load time: {round(hdfs_load_time2, 2)} seconds")

        # Phase 4: Route Cost Evaluation (Triggers distributed simulation)
        calc_start = time.time()
        travel_time_results = calculate_travel_times(spark, lookup_df, matching_dates, route_gantries, args.time)
        calc_time = round(time.time() - calc_start, 2)
        print(f"[*] Travel time calculation time: {calc_time} seconds")

        # Phase 5: Statistics & Output Dumps
        if travel_time_results:
            ...
            print(f"\n================ Result ================")
            print(f"Median time: {median_minutes} minutes")
            ...
            # 直接以內建的 SEGMENT_LOCATIONS dict 查詢路段經緯度，組合成物件形式輸出
            json_data = {}
            ...
            print("\n[!]no any traveltime has been caculate")

    finally:
        # Release dataset persistence states
        if 'lookup_df' in locals():
            lookup_df.unpersist()
        spark.stop()

    end_time = time.time()
    execution_time = round(end_time - start_time, 2)
    print(f"\nTotal script execution time: {execution_time} seconds")
