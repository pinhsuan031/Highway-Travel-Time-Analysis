...
# =====================================================================
def _run_query_core(spark, origin, destination, weekday, depart_time):
...
# =====================================================================
# core1 版本:每次查詢「自己建 session、跑完就 stop」
#
# 目的:當作對照組。travel_time_api.py 只要改成
#   from . import travel_time_core1 as core
# 就會走這條「每次重開連線」的路,拿來跟常駐版比耗時。
#
# 簽名跟常駐版一模一樣 run_query(spark, ...),但「刻意忽略」傳進來的
# spark 參數 —— 因為這版的重點就是不用常駐 session,自己開一個。
# port 用 40010/40011/4041 錯開常駐 session,兩者可並存不撞 port。
# =====================================================================
def _build_fresh_spark():
    return (
        SparkSession.builder
        .appName("TDCS_TravelTime_Fresh")
        ...
        .config("spark.driver.port", "40010")
        .config("spark.blockManager.port", "40011")
        .config("spark.ui.port", "4041")
        .config("spark.driver.memory", os.getenv("SPARK_DRIVER_MEMORY", "4g"))
        .config("spark.executor.memory", os.getenv("SPARK_EXECUTOR_MEMORY", "5g"))
        .config("spark.executor.cores", os.getenv("SPARK_EXECUTOR_CORES", "8"))
        .config("spark.dynamicAllocation.enabled", "true")
        .config("spark.dynamicAllocation.minExecutors", "2")
        .config("spark.dynamicAllocation.maxExecutors", "9")
        .config("spark.dynamicAllocation.executorIdleTimeout", "120s")
        .config("spark.sql.shuffle.partitions", os.getenv("SPARK_SHUFFLE_PARTITIONS", "24"))
...
def run_query(spark, origin, destination, weekday, depart_time):
    """對照組:忽略傳入的 spark,每次自己建 session、跑完 stop。
    回傳格式與常駐版完全相同,額外在 query_seconds 一欄反映「含連線」的總時間。
    """
    total_start = time.time()

    conn_start = time.time()
    fresh_spark = _build_fresh_spark()   # ← 每次重開連線就發生在這
    conn_seconds = round(time.time() - conn_start, 2)
    print(f"[core1] fresh spark connection: {conn_seconds}s", flush=True)

    try:
        result = _run_query_core(
            fresh_spark, origin, destination, weekday, depart_time)
    finally:
        fresh_spark.stop()   # ← 關鍵:跑完就關,不常駐

    if result is not None:
        result["conn_seconds"] = conn_seconds
        result["total_seconds"] = round(time.time() - total_start, 2)
    return result
